...
- **Interactivity**:
  ...
  - Dedicated bookmark toggle button (`.bookmark-btn`) with prominent `w-5.5 h-5.5` SVG icon, amber active/hover styling, and capture-phase event listeners to save materials to offline local bookmarks without triggering card navigation.
  - Dedicated studied / done checkbox toggle button (`.studied-btn`) with prominent `w-6 h-6` checkbox, emerald theme styling, spring pop micro-animation (`.animate-studied-pop`, `.animate-checkmark`), dynamic localized tooltips (`title` / `aria-label`), and instant `localStorage` persistence (`asumed_studied_materials`) with memory fallback. Uses centralized capture-phase click interception to eliminate card link interference and ensure reliable toggle state sync across all active views.
